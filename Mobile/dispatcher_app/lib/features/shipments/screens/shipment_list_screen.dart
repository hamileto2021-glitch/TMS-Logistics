...
class _ShipmentListScreenState extends State<ShipmentListScreen> {
  ...
  Future<void> _deleteShipment(Shipment shipment) async {
    final confirmed = await ConfirmDeleteDialog.show(
      context,
      title: "Delete Shipment",
      message: "Delete ${shipment.shipmentNumber} ?",
    );

    if (!confirmed) return;

    try {
      await _service.deleteShipment(shipment.id);
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          ...
          content: Text("${shipment.shipmentNumber} deleted."),
        ),
      );
  ...
  List<Shipment> _filterShipments(List<Shipment> shipments) {
    ...
    return shipments.where((s) {
      ...
      return s.shipmentNumber.toLowerCase().contains(q) ||
          ...
          s.cargoType.toLowerCase().contains(q);
    }).toList();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(title: const Text("Shipments"), centerTitle: true),
            ...
            child: FutureBuilder<List<Shipment>>(
              ...
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  return const AppLoading(message: "Loading shipments...");
                }
                                ...
                                subtitle: Padding(
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      AppStatusChip(status: shipment.status),
                                    ],
                                  ),
                                ),
                                ...
                                trailing: PopupMenuButton<String>(
                                  onSelected: (value) async {
                                    switch (value) {
                                      ...
                                      case "delete":
                                        await _deleteShipment(shipment);
                                        break;
                                    }
                                  },
